...
export default {
  data() {
    return {
      availableFunds_option: 100,
      currentExpenses_option: 0,
      enteredExpense_option: 0,
    };
  },
  computed: {
    remainingFunds_option() {
      return this.availableFunds_option - this.currentExpenses_option;
    },
  },
  methods: {
    addExpense_option() {
      this.currentExpenses_option += this.enteredExpense_option;
    },
  },
  watch: {
    remainingFunds_option(val) {
      if (val < 0) {
        alert('You are broke!');
      }
    },
