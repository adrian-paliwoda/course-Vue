  ...
  <div>
    ...
    <base-card>
      <form @submit.prevent="submitForm">
        ...
        <base-button mode="flat" type="button" @click="switchMode">{{ switchModeButtonCaption }}</base-button>
      </form>
    </base-card>
  </div>
</template>
...
export default defineComponent({
  components: {BaseSpinner, BaseDialog, BaseButton},
  data() {
    return {
      email: '',
      password: '',
  ...
  methods: {
    submitForm: async function () {
      ...
      this.formIsValid = !(this.email === '' || !this.email.includes('@') || this.password < 6);
